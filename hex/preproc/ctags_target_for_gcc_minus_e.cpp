# 1 "C:\\Users\\ADMIN\\Documents\\Arduino\\ledcube\\ledcube.ino"
# 1 "C:\\Users\\ADMIN\\Documents\\Arduino\\ledcube\\ledcube.ino"
//byte DS=A1;
//byte STCP=A2;
//byte SHCP=A3;
...
void setup() {
 Serial.begin(9600);
    pinMode(DS, 0x1);//ds
    pinMode( SHCP, 0x1);//stcp
    pinMode( STCP, 0x1);//shcp
  for (int x = 0; x < 8; x++) {
    pinMode(Tangz[x], 0x1);
  }
//  while (!Serial) {
...
void dich74hc595(byte a){
    if (a==0) {
        digitalWrite(DS, 0x0);
        digitalWrite(SHCP, 0x1);
        digitalWrite(SHCP, 0x0);
    } else if(a==1) {
        digitalWrite(DS, 0x1);
        digitalWrite(SHCP, 0x1);
        digitalWrite(SHCP, 0x0);
...
void mo74hc595() {
    digitalWrite(STCP, 0x1);
    digitalWrite(STCP, 0x0);
}
void reset() {
    for (byte t=0; t<64; t++) {
        dich74hc595(0);
...
void hienthi(byte mangba[8][8][8], byte Solan){
      for (byte t=0; t<Solan; t++) { // so 7 la thoi gian qua 1 tang
        ...
        for (byte z=0; z<8; z++) {
          ...
          mo74hc595();
          digitalWrite(Tangz[z], 0x1);
          reset();
          digitalWrite(Tangz[z], 0x0);
        }
      }
}
